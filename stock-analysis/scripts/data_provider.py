...
def alpha_vantage_series(ticker: str, function: str, key: str | None) -> dict:
    if not key:
    ...
    url = "https://www.alphavantage.co/query?" + urllib.parse.urlencode(
        {"function": function, "symbol": ticker, "outputsize": "compact", "apikey": key}
    )
    try:
        data = get_json(url)
    except Exception as exc:  # noqa: BLE001
...
def get_daily_ohlcv(ticker: str, keys: dict[str, str], days: int = 252) -> dict:
    attempts = []
    direct = alpha_vantage_series(ticker, "TIME_SERIES_DAILY", keys.get("ALPHA_VANTAGE_API_KEY"))
    if direct.get("status") == "ok":
    ...
    yf_daily = yfinance_history(ticker, f"{days}d", "1d", "yfinance_daily")
