...
description: Use when stock analysis needs bull/bear debate, quant review, risk-manager review, thesis challenge, confidence calibration, or investment committee synthesis. In Claude Code, dispatches real parallel subagents per role per round; falls back to single-LLM simulation only when subagent dispatch is unavailable.
---
...
| Slot | Persona swaps |
|---|---|
| Bull | `stock-investor-wood` (disruptive growth), `stock-investor-druckenmiller` (macro tailwind growth), `stock-investor-lynch` (GARP), `stock-investor-fisher` (long-term quality growth) |
| Bear | `stock-investor-burry` (deep-value short-side discipline), `stock-investor-graham` (defensive quantitative), `stock-investor-buffett` (when valuation is stretched), `stock-investor-munger` (when business quality is questionable) |
...
| Moderator | usually unchanged, or `stock-investor-munger` for opinionated synthesis |
...
1. The persona content the subagent will adopt — for persona slots, **copy the relevant `stock-investor-<persona>/SKILL.md` content inline into the prompt**, do not rely on the subagent discovering the skill on its own
...
<full content of stock-investor-<persona>/SKILL.md, inline>
